...
import { runDailyPass } from '../../_lib/schedule';
...
export async function GET(request) {
  ...
  return Response.json(await runDailyPass('main', { fromCron: true }));
}
export async function POST(request) {
  if (!kvConfigured()) return Response.json({ error: 'KV not configured' }, { status: 500 });
  if (!sameOrigin(request)) return Response.json({ error: 'Forbidden' }, { status: 403 });
  const body = await request.json().catch(() => ({}));
  return Response.json(await runDailyPass(wsFrom(request, body)));
}
