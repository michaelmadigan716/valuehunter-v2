              ...
              <div className="mb-3 p-3 rounded-lg border" style={{ background: 'rgba(251,191,36,0.05)', borderColor: 'rgba(251,191,36,0.25)' }}>
                <div className="flex items-start justify-between gap-3">
                  <div>
                    ...
                    <p className="text-xs text-slate-500 mt-0.5">Cuts the big fast-model passes down to what's worth paying for. <span className="text-slate-400">Stage 0:</span> a cheap batched Singularity relevance score (one call per ~20 stocks) is taken first; no per-stock AI runs unless Singularity ≥ {appSettings.staging?.minSingularity ?? 50}, or an insider bought in the last {appSettings.staging?.insiderDays ?? 90} days, or technicals ≥ {appSettings.staging?.minTechScore ?? 85}%.{appSettings.staging?.minComposite != null ? ` Composite ≥ ${appSettings.staging.minComposite} also passes.` : ''} <span className="text-slate-400">Stage 1:</span> the 3 cheap scans (Conviction, Technical, Valuation - no live search). <span className="text-slate-400">Stage 2:</span> the 4 live-search scans (Momentum, Buyout, Leadership, Playbook) only if stage 1 shows Conviction ≥ {appSettings.staging?.stage2?.insiderConviction ?? 50}, Valuation ≥ {appSettings.staging?.stage2?.valuationScore ?? 60} or Cup&amp;Handle ≥ {appSettings.staging?.stage2?.cupHandleScore ?? 60} (or the insider/technicals signals above). Daily targeted and smart re-scans always run everything.</p>
                  </div>
                  <button
                    onClick={async () => {
