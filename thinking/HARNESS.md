...
- The repository is cloned in the working directory. Read `thinking/categories/<category>.md` for the category brief. Never commit, never modify the repo.
- Tools: Bash (`curl`), WebSearch, WebFetch, Read. Use web research heavily. Prefer primary sources: SEC filings (10-K/10-Q/8-K/Form 4), earnings transcripts, company IR pages, supplier/customer announcements, reputable trade press. Social media (X, Reddit) only as sentiment signal, never as fact.
...
- Do not modify the repository or open pull requests.
