...
// Singularity relevance, scored in cheap batches (one call per ~20 stocks, no
// live search). This is the primary free-ish gate before any per-stock scan.
export async function scoreSingularityBatch(stocks, model = 'grok-4.3') {
  if (!stocks.length) return {};
  const stockList = stocks.map(s => `${s.ticker}: ${s.name || ''} (${s.sector || 'Unknown'})`).join('\n');
...
- Also count: AI software/infrastructure, autonomy, space, defense tech, biotech using AI at its core
Score 80-100 = core enabler of the trend; 50-79 = meaningful supplier or adopter; 20-49 = tangential; 0-19 = unrelated (banks, retail, food, REITs, insurance...).

CATEGORY DETECTION - true if the company is primarily in these sectors:
- isBank, isFood, isHealthcare, isInsurance, isREIT

STOCKS:
${stockList}

Respond with ONLY JSON: {"stocks":[{"ticker":"ABC","singularity":85,"isBank":false,"isFood":false,"isHealthcare":false,"isInsurance":false,"isREIT":false}]}`;
  try {
    const response = await fetch(`${API_BASE}/api/grok`, {
      method: 'POST', headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ prompt, model, jsonMode: true })
    });
    if (!response.ok) throw new Error(`API ${response.status}`);
    const data = await response.json();
    let parsed;
    try { parsed = JSON.parse(data.analysis); } catch { const m = String(data.analysis || '').match(/\{[\s\S]*\}/); parsed = m ? JSON.parse(m[0]) : null; }
    const rows = Array.isArray(parsed) ? parsed : (parsed?.stocks || []);
    const out = {};
    for (const r of rows) {
      if (!r || !r.ticker) continue;
      out[String(r.ticker).toUpperCase()] = {
        singularityScore: Math.min(100, Math.max(0, Number(r.singularity) || 0)),
        isBank: !!r.isBank, isFood: !!r.isFood, isHealthcare: !!r.isHealthcare, isInsurance: !!r.isInsurance, isREIT: !!r.isREIT,
        singularityAt: Date.now(),
      };
    }
    return out;
  } catch (e) {
    console.error('Singularity batch failed:', e);
    return {};
  }
}
